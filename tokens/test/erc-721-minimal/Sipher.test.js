const Testing = artifacts.require("ERC721SipherMinimal.sol");
...
contract("ERC721Sipher minimal", accounts => {
  ...
  beforeEach(async () => {
    ...
    await token.__ERC721Sipher_init(name, "RARI", "https://ipfs.sipher.com", "https://ipfs.sipher.com", whiteListProxy, proxyLazy.address);
    await token.transferOwnership(tokenOwner);
    erc1271 = await ERC1271.new();
  });

  describe("Burn before ERC721SipherMinimal ()", () => {
  ...
  describe("Burn after ERC721SipherMinimal ()", () => {
  ...
  it("mint and transfer by minter, and token created by ERC721Factory ", async () => {
    ...
    resultCreateToken = await factory.methods['createToken(string,string,string,string,uint256)']("name", "RARI", "https://ipfs.sipher.com", "https://ipfs.sipher.com", 1, {from: tokenOwner});
