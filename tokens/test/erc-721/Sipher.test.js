const Testing = artifacts.require("ERC721Sipher.sol");
...
const ERC721Factory = artifacts.require("ERC721SipherFactoryC2.sol");
...
contract("ERC721Sipher", (accounts) => {
  let token;
  ...
  beforeEach(async () => {
    ...
    await token.__ERC721Sipher_init(
      name,
      "RARI",
      "https://ipfs.Sipher.com",
      "https://ipfs.Sipher.com",
      whiteListProxy,
      proxyLazy.address
    );
  ...
  describe("Burn before ERC721Sipher ()", () => {
    it("Run burn from minter, mintAndTransfer by the same minter not possible, token burned, throw", async () => {
      ...
      await expectThrow(
        //try to mint and transfer token, throw, because token was burned
    ...
    it("Run burn from another, throw, mintAndTransfer by the same minter is possible", async () => {
      ...
      const tokenURI = "//uri";
      await expectThrow(
        //another burn item, in tokenId minter address contains, throw
        token.burn(tokenId, { from: transferTo })
      );
      //mint and transfer token, ok, because token was not burned, possible to mint to a new user
      await token.mintAndTransfer([tokenId, tokenURI, creators([minter]), [], [zeroWord]], transferTo, {
        from: minter,
      });
  ...
  describe("Burn after ERC721Sipher ()", () => {
    it("Run mintAndTransfer, burn, mintAndTransfer by the same minter, throw", async () => {
      ...
      await expectThrow(
        //try once more mint and transfer
    ...
    it("Run transferFromOrMint, burn, transferFromOrMint by the same minter, throw", async () => {
      ...
      await token.transferFromOrMint([tokenId, tokenURI, creators([minter]), [], [zeroWord]], minter, transferTo, {
        from: minter,
      });
      assert.equal(await token.ownerOf(tokenId), transferTo);
      await token.burn(tokenId, { from: transferTo });
      await expectThrow(
        token.transferFromOrMint([tokenId, tokenURI, creators([minter]), [], [zeroWord]], minter, transferTo, {
          from: minter,
        })
      );
    });
  ...
  it("mint and transfer by minter, and token created by ERC721Factory ", async () => {
    ...
    const resultCreateToken = await factory.methods["createToken(string,string,string,string,uint256)"](
      "name",
      "RARI",
      "https://ipfs.Sipher.com",
      "https://ipfs.Sipher.com",
    ...
    const tx = await tokenByProxy.mintAndTransfer([tokenId, tokenURI, creators([minter]), [], [zeroWord]], minter, {
      from: minter,
    });
  ...
  it("checkPrefix should work correctly, checks for duplicating of the base part of the uri ", async () => {
    beacon = await UpgradeableBeacon.new(token.address);
    factory = await ERC721Factory.new(beacon.address, transferProxy.address, proxyLazy.address);
    const baseURI = "https://ipfs.Sipher.com";
    const resultCreateToken = await factory.methods["createToken(string,string,string,string,uint256)"](
      "name",
      "RARI",
      baseURI,
    ...
    await tokenByProxy.mintAndTransfer([tokenId, tokenURI, creators([minter]), [], [zeroWord]], minter, {
      from: minter,
    });
    ...
    await tokenByProxy.mintAndTransfer([tokenId1, tokenURI1, creators([minter]), [], [zeroWord]], minter, {
      from: minter,
    });
    ...
    await tokenByProxy.mintAndTransfer([tokenId2, tokenURI2, creators([minter]), [], [zeroWord]], minter, {
      from: minter,
    });
  ...
  it("check Royalties IERC2981, with 3 royaltiesBeneficiary ", async () => {
    ...
    const tx = await token.mintAndTransfer(
      [
        tokenId,
        tokenURI,
        creators([minter]),
  ...
  it("check Royalties IERC2981, with 3 royaltiesBeneficiary zero fee, throw ", async () => {
    ...
    const signature = await getSignature(
      ...
      tokenURI,
      creators([minter]),
      feesWithZero([royaltiesBeneficiary1, royaltiesBeneficiary2, royaltiesBeneficiary3]),
      minter
    );
  ...
  it("check Royalties IERC2981, with only 1 royaltiesBeneficiary ", async () => {
    ...
    const tx = await token.mintAndTransfer(
      [tokenId, tokenURI, creators([minter]), fees([royaltiesBeneficiary1]), [signature]],
      transferTo,
      { from: whiteListProxy }
    );
    const addressValue = await token.royaltyInfo(tokenId, WEIGHT_PRICE);
  ...
  it("check Royalties IERC2981, with only 0 royaltiesBeneficiary ", async () => {
    ...
    const tx = await token.mintAndTransfer([tokenId, tokenURI, creators([minter]), fees([]), [signature]], transferTo, {
      from: whiteListProxy,
    });
    const addressValue = await token.royaltyInfo(tokenId, WEIGHT_PRICE);
  ...
  it("mint and transfer by whitelist proxy. several creators", async () => {
    ...
    await token.mintAndTransfer(
      [tokenId, tokenURI, creators([minter, creator2]), fees, [signature1, signature2]],
      transferTo,
      { from: whiteListProxy }
    );
  ...
  it("mint and transfer by whitelist proxy. several creators. minter is not first", async () => {
    ...
    await expectThrow(
      token.mintAndTransfer(
        [tokenId, tokenURI, creators([creator2, minter]), fees, [signature2, signature1]],
        transferTo,
        { from: whiteListProxy }
      )
    );
  ...
  it("mint and transfer by whitelist proxy. several creators. wrong order of signatures", async () => {
    ...
    await expectThrow(
      token.mintAndTransfer(
        [tokenId, tokenURI, creators([minter, creator2]), fees, [signature2, signature1]],
        transferTo,
        { from: whiteListProxy }
      )
    );
  ...
  it("mint and transfer by approved proxy for all", async () => {
    ...
    const ttxx = await token.mintAndTransfer([tokenId, tokenURI, creators([minter]), [], [signature]], transferTo, {
      from: proxy,
    });
  ...
  it("mint and transfer by approved proxy for tokenId", async () => {
    ...
    await expectThrow(token.approve(proxy, tokenId, { from: minter }));
  ...
  it("transferFromOrMint when not minter. not yet minted", async () => {
    ...
    await token.transferFromOrMint([tokenId, tokenURI, creators([minter]), [], [zeroWord]], transferTo, accounts[5], {
      from: transferTo,
    });
  ...
  it("mint and transfer with signature of not minter", async () => {
    ...
    await expectThrow(
      token.mintAndTransfer([tokenId, tokenURI, creators([minter]), [], [signature]], transferTo, {
        from: whiteListProxy,
      })
    );
  });
  ...
  it("standard transfer by not approved contract", async () => {
    ...
    let transferTo = accounts[2];
    await expectThrow(token.transferFrom(minter, transferTo, tokenId, { from: accounts[8] }));
  });
  ...
  it("signature by contract wallet erc1271, with whitelist proxy", async () => {
    ...
    await expectThrow(
      token.mintAndTransfer([tokenId, tokenURI, creators([minter.address]), [], [zeroWord]], transferTo, {
        from: whiteListProxy,
      })
    );

    await erc1271.setReturnSuccessfulValidSignature(true);
    await token.mintAndTransfer([tokenId, tokenURI, creators([minter.address]), [], [zeroWord]], transferTo, {
      from: whiteListProxy,
    });
    assert.equal(await token.ownerOf(tokenId), transferTo);
  });
