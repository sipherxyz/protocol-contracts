const Testing = artifacts.require("ERC1155Sipher.sol");
...
const ERC1155SipherFactoryC2 = artifacts.require("ERC1155SipherFactoryC2.sol");
...
contract("ERC1155Sipher", accounts => {
  ...
  beforeEach(async () => {
    ...
    await token.__ERC1155Sipher_init(name, "TST", "ipfs:/", "ipfs:/", whiteListProxy, proxyLazy.address, {from: tokenOwner});
...
//      event BurnLazy(address indexed operator, address indexed account, uint256 id, uint256 amount);
  ...
  describe("Mint and transfer  ()", () => {
    it("mint and transfer by minter, token create by Factory", async () => {
      ...
      factory = await ERC1155SipherFactoryC2.new(beacon.address, transferProxy.address, proxyLazy.address);
      const salt = 3;
      ...
      truffleAssert.eventEmitted(resultCreateToken, 'Create1155SipherProxy', (ev) => {
        proxy = ev.proxy;
        return true;
      });
      ...
      truffleAssert.eventEmitted(resultCreateToken2, 'Create1155SipherProxy', (ev) => {
          addrToken2 = ev.proxy;
        return true;
      });
      ...
      truffleAssert.eventEmitted(resultCreateToken3, 'Create1155SipherProxy', (ev) => {
        addrToken3 = ev.proxy;
      return true;
      });
    ...
    it("checkPrefix should work correctly, checks for duplicating of the base part of the uri ", async () => {
      ...
      const resultCreateToken = await factory.methods['createToken(string,string,string,string,uint256)']("name", "RARI", baseURI, "https://ipfs.sipher.com", 1, {from: tokenOwner});
      truffleAssert.eventEmitted(resultCreateToken, 'Create1155SipherProxy', (ev) => {
         proxy = ev.proxy;
        return true;
      });
