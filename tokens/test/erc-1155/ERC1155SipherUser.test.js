...
const ERC1155SipherUserFactoryC2 = artifacts.require("ERC1155SipherFactoryC2.sol");
...
contract("ERC1155SipherUser", accounts => {
  ...
  beforeEach(async () => {
    ...
    await token.__ERC1155SipherUser_init(name, "TST", "ipfs:/", "ipfs:/", [whiteListProxy], accounts[6], accounts[7], {from: tokenOwner});
  });
  ...
  it("mint and transfer by minter, token create by Factory", async () => {
    ...
    factory = await ERC1155SipherUserFactoryC2.new(beacon.address, ZERO, ZERO);
    const salt = 3;
    ...
    truffleAssert.eventEmitted(resultCreateToken, 'Create1155SipherUserProxy', (ev) => {
     	proxy = ev.proxy;
      return true;
    });
    ...
    truffleAssert.eventEmitted(resultCreateToken2, 'Create1155SipherUserProxy', (ev) => {
        addrToken2 = ev.proxy;
      return true;
    });
    ...
    truffleAssert.eventEmitted(resultCreateToken3, 'Create1155SipherUserProxy', (ev) => {
      addrToken3 = ev.proxy;
    return true;
    });
