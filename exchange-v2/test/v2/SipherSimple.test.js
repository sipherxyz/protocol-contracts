...
const SipherSimpleTest = artifacts.require("SipherSimpleTest.sol");
...
contract("SipherSimpleTest:", (accounts) => {
  let testing;
  ...
  beforeEach(async () => {
    testing = await SipherSimpleTest.new();
  });

  describe("Check royalties()", () => {
    it("Check get royalties ERC1155Lazy ", async () => {
      let encodedMintData = await testing.encode1155([
        1,
        "uri",
        10,
        [
    ...
    it("Check  get royalties  ERC721Lazy ", async () => {
      let encodedMintData = await testing.encode721([
        1,
        "uri",
        [
          [accounts[1], 0],
