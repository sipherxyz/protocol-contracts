...
contract("RoyaltiesRegistry, royalties types test", (accounts) => {
  ...
  describe("royalties types are set correctly", () => {
    ...
    it("test royalties type = 2, royalties v2", async () => {
      const ERC721_V2OwnUpgrd = await TestERC721RoyaltyV2OwnUpgrd.new("Sipher", "RARI", "https://ipfs.sipher.com", {
        from: ownerErc721,
      });
    ...
    it("test royalties type = 3, royalties v1", async () => {
      const ERC721_V1OwnUpgrd = await TestERC721RoyaltyV1OwnUpgrd.new("Sipher", "RARI", "https://ipfs.sipher.com", {
        from: ownerErc721,
      });
    ...
    it("test royalties type = 5, royalties 2981", async () => {
      ...
      const ERC721_V2981 = await TestERC721WithRoyaltiesV2981.new("Sipher", "RARI", "https://ipfs.sipher.com", {
        from: ownerErc721,
      });
  ...
  describe("royalties types set correctly from external methods", () => {
    ...
    it("forceSetRoyaltiesType + clearRoyaltiesType", async () => {
      ...
      await expectThrow(royaltiesRegistry.forceSetRoyaltiesType(token, 1, { from: accounts[1] }));
      console.log(2);
      //can't set royalties type to 0
      await expectThrow(royaltiesRegistry.forceSetRoyaltiesType(token, 0));
      console.log(4);
      ...
      await expectThrow(royaltiesRegistry.forceSetRoyaltiesType(token, 7));
      console.log(5);
      //only owner can clear royalties
      await expectThrow(royaltiesRegistry.clearRoyaltiesType(token, { from: accounts[3] }));
      console.log(6);
      //clearRoyaltiesType
      await royaltiesRegistry.clearRoyaltiesType(token);
      assert.equal(await royaltiesRegistry.getRoyaltiesType(token), 0, "clearRoyaltiesType ");
  ...
  describe("upgrade checks", () => {
    it("check storage after upgrade", async () => {
      ...
      const royaltiesRegistryOld = await deployProxy(RoyaltiesRegistryOld, [], {
      ...
      assert.equal(
        (await royaltiesRegistry.getRoyalties.call(token, tokenId3)).length,
        0,
        "royaltiesFromTokenAndTokenId"
      );

      assert.equal(
        (await royaltiesRegistry.getRoyalties.call(token2, tokenId3))[0].accounts,
        royaltiesFromToken[0].accounts,
        "royaltiesFromToken"
      );
      assert.equal(
        (await royaltiesRegistry.getRoyalties.call(token2, tokenId3))[0].value,
        royaltiesFromToken[0].value,
        "royaltiesFromToken"
      );

      assert.equal(
        (await royaltiesRegistry.getRoyalties.call(token3, defaultTokenId1))[0].accounts,
        royaltiesFromProvider[0].accounts,
        "royaltiesFromProvider"
      );
      assert.equal(
        (await royaltiesRegistry.getRoyalties.call(token3, defaultTokenId1))[0].value,
